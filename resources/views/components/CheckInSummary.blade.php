<div class="text-left text-sm text-gray-600" data-check-in-summary>
    @if ($checkinData && !empty($checkinData))
        ...
        <div class="mb-4 py-2 border-b border-green">
            <div class="text-xs text-gray-500">Receipt ID: <span data-receipt-id>{{ $checkinData['id'] ?? 'N/A' }}</span>
            </div>
            <div class="text-sm flex justify-between pt-1 text-gray-500">
                Check-in Date:
                <strong><span
                        data-receipt-date>{{ $checkinData['date'] ? \Carbon\Carbon::parse($checkinData['date'])->toFormattedDateString() : now()->toFormattedDateString() }}</span></strong>
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4 border-b mb-4">
            {{-- Owner Information --}}
            @if (isset($checkinData['user']['info']))
                <div class="pb-1" data-owner-info>
                    <h3 class="font-semibold text-green-dark mb-1">👤 Owner Information</h3>
                    <div class="pl-4 text-sm">
                        <div class="flex justify-between"><strong>Name:</strong>
                            <span data-owner-name>{{ $checkinData['user']['info']['name'] ?? 'Not provided' }}</span>
                        </div>
                        <div class="flex justify-between"><strong>Phone:</strong>
                            <span data-owner-phone>{{ $checkinData['user']['info']['phone'] ?? 'Not provided' }}</span>
                        </div>
                        <div class="flex justify-between"><strong>Email:</strong>
                            <span data-owner-email>{{ $checkinData['user']['info']['email'] ?? 'Not provided' }}</span>
                        </div>
                        @if (isset($checkinData['user']['info']['address']))
                            <div class="flex justify-between"><strong>Address:</strong>
                                <span data-owner-address>{{ $checkinData['user']['info']['address'] }}</span>
                            </div>
                        @endif
                        @if (isset($checkinData['user']['info']['city']) && isset($checkinData['user']['info']['zip']))
                            <div class="flex justify-between"><strong>Location:</strong>
                                <span data-owner-location>{{ $checkinData['user']['info']['city'] }},
                                    {{ $checkinData['user']['info']['zip'] }}</span>
                            </div>
                        @endif
                    </div>
                </div>
            ...
            @if (isset($checkinData['user']['emergencyContact']) &&
                ...
                <div class="" data-emergency-contact>
                    <div class="font-semibold text-green-dark mb-1">🚨 Emergency Contact</div>
                    <div class="pl-4 text-sm">
                        <div class="flex justify-between"><strong>Name:</strong>
                            <span
                                data-emergency-name>{{ $checkinData['user']['emergencyContact']['name'] ?? 'Not provided' }}</span>
                        </div>
                        <div class="flex justify-between"><strong>Phone:</strong>
                            <span
                                data-emergency-phone>{{ $checkinData['user']['emergencyContact']['phone'] ?? 'Not provided' }}</span>
                        </div>
                    </div>
                </div>
            @endif
                                    ...
                                    <div>
                                        ...
                                        @if (isset($pet['feeding']) && is_array($pet['feeding']) && count($pet['feeding']) > 0)
                                            ...
                                            @foreach ($feedingByTime as $time => $items)
                                                <p class="pl-2 flex justify-between text-xs">
                                                    <strong>{{ ucfirst($time) }}:</strong>
                                                    {{ implode(', ', $items) }}
                                                </p>
                                            @endforeach
                                        @endif
                                    ...
                                    @if (isset($checkinData['inventory']) && is_array($checkinData['inventory']))
                                        <div class="mb-4" data-inventory-section>
                                            <div class="font-semibold text-yellow-800 mb-1">🎒 Items to Store
                                                (<span
                                                    data-inventory-count>{{ count($checkinData['inventory']) }}</span>
                                                item<span
                                                    data-inventory-plural>{{ count($checkinData['inventory']) > 1 ? 's' : '' }}</span>)
                                            </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 my-4">
                                    ...
                                    @if (isset($checkinData['grooming']) && is_array($checkinData['grooming']))
                                        ...
                                        @if ($hasGroomingServices)
                                            <div class="mb-4" data-grooming-section>
                                                <div class="font-semibold text-yellow-800 mb-1">🛁 Grooming Services
                                                </div>
                                                <div class="pl-4 text-sm">
                                                    @php
                                                        $services = [];
                                                ...
                                                @if (isset($checkinData['grooming']['appointmentDay']) && $checkinData['grooming']['appointmentDay'])
                                                    <div class="pl-4 text-sm mt-1">
                                                        <strong>📅 Appointment Day:</strong> <span
                                                            data-grooming-appointment>{{ $checkinData['grooming']['appointmentDay'] }}</span>
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
                                    ...
                                    @if (isset($checkinData['groomingDetails']))
                                        <div class="mb-4" data-grooming-instructions-section>
                                            <div class="font-semibold text-yellow-800 mb-1">📝 Grooming Instructions
                                            </div>
                                            <div class="pl-4 text-sm" data-grooming-instructions>
                                                {{ $checkinData['groomingDetails'] }}</div>
                                        </div>
                                    @endif
                                </div>
                            </div>
    ...
    @else
        <div class="p-4 bg-yellow-50 border border-yellow-200 rounded">
            <p class="text-yellow-800">⚠️ No check-in data available. Please complete your check-in first.</p>
        </div>
    @endif
</div>

                        ...
                        <div class="border border-blue-200 rounded-lg p-4 bg-blue-50">
                            <div class="flex justify-between items-start mb-3">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">Type: {{ $checkIn->pet->kindOfPet->name ?? 'N/A' }}</p>
                            ...
                            <div class="flex gap-2">
                                <form action="{{ route('pet-staff.checkout', $checkIn->id) }}" method="POST" class="flex-1">
                                    @csrf
                                ...
                                <form action="{{ route('pet-staff.cancel', $checkIn->id) }}" method="POST" class="flex-1">
                                    @csrf
                                    <button type="submit" class="w-full px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition" onclick="return confirm('Are you sure you want to cancel this check-in?')">
                                        Cancel
                                    </button>
                                </form>
                            </div>
                    ...
                    @foreach ($droppedInPets as $checkIn)
                        <div class="border border-orange-200 rounded-lg p-4 bg-orange-50">
                            <div class="flex justify-between items-start mb-3">
                                ...
                                <span class="px-3 py-1 bg-orange-200 text-orange-800 rounded-full text-xs font-semibold">
                                    DROPPED-IN
                                </span>
                            </div>
